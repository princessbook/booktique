...
const ArticleCommentInput = ({ postId }: { postId: string }) => {
  ...
  const handleSubmit = async () => {
    if (!isWrite) {
      console.log('작성해라'); //NOTE - 토스트ui 변경 필요 유저 id가 없으면 막아야한다 근데 클럽 가입에서 막나?
      return;
    }
  ...
  return (
    <div className='w-full flex h-[74px] bg-grayBg sticky bottom-0'>
